...
class EdnDoc
  """
  Top level EDN type containing an entire document.
  A EDN document consists of exactly 1 value.
  """
  var data: EdnType
  """
  The parsed EDN structure.
  ...
  new create() =>
    """
    Default constructor building a document containing a single nil.
    """
    data = None
  ...
  fun string(indent: String = "", pretty_print: Bool = false): String =>
    ...
    """
    let buf =
      _EdnPrint._string(data, recover String(256) end, indent, 0,
        pretty_print)
    buf.compact()
    buf
  ...
  fun ref parse(source: String) ? =>
    """
    Parse the given string as an EDN file, building a document.
    Raise error on invalid EDN in given source.
    """
    _source = source
    _index = 0
    _line = 1
  ...
  fun ref _parse_value(context: String): EdnType ? =>
    """
    Parse a single EDN value of any type, which MUST be present.
    Raise error on invalid or missing value.
    """
    _dump_whitespace()
    match _peek_char(context)?
    | let c: U8 if (c >= 'a') and (c <= 'z') => _parse_bool_nil()?
  ...
  fun ref _parse_bool_nil(): (Bool | None) ? =>
    """
  ...
  fun ref _parse_number(): (F64 | I64) ? =>
    ...
    if _peek_char()? == 'N' then
      // We have an N at the end of the int
      _get_char()? // Consume N
    end
  ...
  fun ref _parse_object(): EdnObject ? =>
    """
    Parse an EDN object, the leading { of which has already been peeked.
    """
    _get_char()? // Consume {
    _dump_whitespace()
    ...
    let map = Map[String, EdnType]

    // Find elements in object
    while true do
      // Each element of of the form:
      //   :key value
      let key = _parse_keyword()?
      _dump_whitespace()

      map(key) = _parse_value("object")?
      _dump_whitespace()

      // Must now have another element, separated by a space or comma, or the end of the
      // object
      match _get_char("object")?
      | '}' => break // End of object
      | ' ' => None  // Next element
      | ',' => None  // Next element
      | ':' => None  // Next element is a key
      else
    ...
    EdnObject.from_map(map)

  fun ref _parse_array(): EdnArray ? =>
    """
    ...
    EdnArray.from_array(array)
  
    fun ref _parse_keyword(): String ? =>
    """
    Parse a keyword with the leading character of :
    """
    _dump_whitespace()

    let peek = _peek_char("keyword")?

    if peek == ':' then
      _get_char()? // Consume :
    elseif (peek == ',') or (peek == '}') then
      _error("Expected a keyword, got '" + _last_char() + "'")
      error
    end
    ...
    while let c = _peek_char("keyword")?; c != ' ' do
      text.push(c)
      _get_char()? // Consume peeked char
    end
